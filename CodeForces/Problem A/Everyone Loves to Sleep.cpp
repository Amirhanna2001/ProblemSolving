...
void AmirHanna2001(){
    ...
    for(auto &hhh : arra) {
        cin >> hhh.first >> hhh.second;
    }
    sort(all(arra));
    arra.push_back({arra[0].first + 24, arra[0].second});
    int answ = 24*60+100;
    for(int i = 0; i < arra.size(); i++) {
        if(H < arra[i].first || (arra[i].first == H && (M <= arra[i].second)))
            answ = min(answ, -(H * 60 + M - arra[i].first * 60 - arra[i].second));
    }
